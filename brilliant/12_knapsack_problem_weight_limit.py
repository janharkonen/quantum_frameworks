...
def energy(angles, n):
    weights = np.array([2, 2, 2]) # weights
    values = np.array([500, 14000, 400]) # weights
...
def qiskit():
    ...
    gradient_delta = 0.01
    gradient_coefficient = 0.0001
    n = 100000
    angles_guess_initial = np.array([np.pi/2, np.pi/2, np.pi/2, np.pi/2, np.pi/2])
    ...
    while True:
        ...
        angles_guess_new = (angles_guess_old - gradient_coefficient*gradient_old) % (2*np.pi)
